...
%% 4) MLE
     ...
     if llf_start == Inf
      ...
      [fit_hat,fval,exitflag,output] = fmincon(@(x)gradl_CDF_Lei(x,  taugrid, nmixtures, y', X'),start,A,b,[],[],lower,upper,[],options);
...
switch truth_start
    ...
    case -4
        save (sprintf('pl_CDF_truth_c2_%d_new',ntau))
    case -3
        save (sprintf('pl_CDF_qreg_3_%d',ntau))
    case -2
        save (sprintf('pl_CDF_qreg_para_%d_new',ntau))
    case -1
        save (sprintf('pl_CDF_truth_para_%d',ntau))
    case 1
        save (sprintf('pl_CDF_truth_%d',ntau))
    otherwise
        save (sprintf('pl_CDF_qreg_%d_new',ntau))
end
