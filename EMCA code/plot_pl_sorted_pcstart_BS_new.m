...
close all;
  ...
  %%% Part A) Set parameters and load data
    ...
    % 10 White 15 tau
    load RData_pl_Jacob10_sub21_ntau15_white100_new
  ...
  %%% Part D) Plotting 
    ...
    plot(taugrid,beta_WLS_start_sorted_full(j_covar+1,:),'k','LineWidth',1);
    output_matrix(2, :) = beta_WLS_start_sorted_full(j_covar+1,:);
    ...
    output_matrix(4, :) = beta_qreg_full(j_covar+1,:) + 1.96*beta_qreg_std(j_covar+1,:);  
    plot(taugrid,beta_qreg_full(j_covar+1,:) - 1.96*beta_qreg_std(j_covar+1,:),'g--');
    output_matrix(5, :) = beta_qreg_full(j_covar+1,:) - 1.96*beta_qreg_std(j_covar+1,:);
    
    plot(taugrid,beta_WLS_start_sorted_full(j_covar+1,:) + 1.96*beta_WLS_start_sorted_std(j_covar+1,:),'k--');
    output_matrix(6, :) = beta_WLS_start_sorted_full(j_covar+1,:) + 1.96*beta_WLS_start_sorted_std(j_covar+1,:); 
    plot(taugrid,beta_WLS_start_sorted_full(j_covar+1,:) - 1.96*beta_WLS_start_sorted_std(j_covar+1,:),'k--');
    output_matrix(7, :) = beta_WLS_start_sorted_full(j_covar+1,:) - 1.96*beta_WLS_start_sorted_std(j_covar+1,:);
     

    plot(taugrid,beta_pc_s_full(j_covar+1,:) + 1.96*beta_pc_s_std(j_covar+1,:),'r--');
    output_matrix(8, :) = beta_pc_s_full(j_covar+1,:) + 1.96*beta_pc_s_std(j_covar+1,:);
    plot(taugrid,beta_pc_s_full(j_covar+1,:) - 1.96*beta_pc_s_std(j_covar+1,:),'r--');
    output_matrix(9, :) = beta_pc_s_full(j_covar+1,:) - 1.96*beta_pc_s_std(j_covar+1,:);
