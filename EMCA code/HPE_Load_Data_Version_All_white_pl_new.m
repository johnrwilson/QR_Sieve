...
% Part B) Load original data
y_orig=data(:,2);
X_orig=[ones(size(y_orig)), data(:,1)];
...
for j = [1:n_repeatsample]   
    ...
    coef_data(:,[1,2]) = fit;
    csvwrite('recorder_qreg.csv',fit)
    ...
    coef_data(:,[3,4]) = betahat';
    csvwrite('WLS.csv',betahat');
    ...
    coef_data(:,[5,6]) = beta_WLS_start_sorted';
    csvwrite('MLE.csv',beta_WLS_start_sorted');
...
coef_write = [taugrid;coef_data']';

csvwrite('coef.csv',coef_write)
save bivar
