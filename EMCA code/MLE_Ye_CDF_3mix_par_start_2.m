...
lower_pl=[zeros(1,(3*ntau)), (zeros(1,(nmixtures-1))+0.001),(zeros(1,(nmixtures-1))-10.01),(zeros(1,nmixtures)+0.01)];
upper_pl=[(zeros(1,(3*ntau))+10), (zeros(1,(nmixtures-1))*0+1),(zeros(1,(nmixtures-1))+10),ones(1,nmixtures)*10];
...
for j_iter = [1:iter]
    ...
    start = max(start, lower_pl);
    start = min(start, upper_pl);
    if gradl_CDF_Lei_GA(start,  taugrid_temp, nmixtures, y', X') == Inf
        exit_recorder_pc_start(j_iter) = -10;
    else
        [fit_hat,fval,exitflag,output] = fmincon(@(x)gradl_CDF_Lei_GA(x,  taugrid_temp, nmixtures, y', X'), start, A, b,[],[],lower_pl-10^(-5),upper_pl+10^(-5),[],options);
        recorder_pc_start(j_iter,:) = fit_hat;
        fval_recorder_pc_start(j_iter) = fval;
        exit_recorder_pc_start(j_iter) = exitflag;
    end
    
    save Ye_GA_3mix_3_par_start_2_1
