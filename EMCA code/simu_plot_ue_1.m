...
switch EIV
    ...
    case '3mix w WLS'
        ...
        load Ye_GA_3mix_3_WLS_ue_99knot taugrid recorder_qreg_repeat_reshape
        pl = 1;
        % case '3mix wo WLS'
    ...
    case 'Laplace PC'
        % Estimating a mixture 2 normals as a mixture of 3 normals.
        load Ye_GA_L_3_WLS_ue
        recorder_WLS_start_repeat_sorted = recorder_WLS_start_repeat_sorted([1:100], :, :);
    ...
    case 'T PC'
        ...
        recorder_WLS_start_repeat_sorted = recorder_WLS_start_repeat_sorted([1:100], :, :);
        load Ye_GA_T_3_WLS_ue_qreg taugrid recorder_qreg_repeat_reshape
...
beta_WLS_start_sorted_upper = quantile(recorder_WLS_start_repeat_sorted,0.975);
beta_WLS_start_sorted_lower = quantile(recorder_WLS_start_repeat_sorted,0.025);
...
if pl == 1
    ...
    beta_pc_s_upper = squeeze(quantile(recorder_pc_start_repeat_recon, 0.975));
    beta_pc_s_lower = squeeze(quantile(recorder_pc_start_repeat_recon, 0.025));
end
...
if pl == 2
    ...
    beta_wls_sorted_upper = squeeze(quantile(recorder_WLS_start_repeat_sorted,0.975));
    beta_wls_sorted_lower = squeeze(quantile(recorder_WLS_start_repeat_sorted,0.025));
end
% For the y-star case
if pl == 3
...
for j_covar = [0:ncovar-1]
    ...
    if pl == 1
        plot(taugrid_ue,beta_pc_s_upper(j_covar+1,:),'g--');
        plot(taugrid_ue,beta_pc_s_lower(j_covar+1,:),'g--');
	...
	switch pl 
    	...
    	case 1
		legend('truth','quantile regression','piecewise linear(fmincon)','2.5%-97.5% CI');
		case 2
