...
%% Part C) Test the gradient function
% C-1) A small disturbance to c
epsilon = 0.001;
para_temp = para;
para_temp(1,1) = para_temp(1,1) + epsilon;
leachsample_c = integral(@(t)fun(para_temp, taugrid, t, taustep, x, y, ParameterDist, nmixtures), 0, 1,'ArrayValued', true)';
lf_c = sum(log(leachsample_c))/N;
g_c = (lf_c-lf)/epsilon;

% C-2) A small disturbance to beta
epsilon = 0.001;
para_temp = para;
para_temp(4,10) = para_temp(4,10) + epsilon;
leachsample_beta = integral(@(t)fun(para_temp, taugrid, t, taustep, x, y, ParameterDist, nmixtures), 0, 1,'ArrayValued', true)';
lf_beta = sum(log(leachsample_beta))/N;
g_beta = (lf_beta-lf)/epsilon;

% C-3) A small disturbance to lambda
ParameterDist_temp = ParameterDist;
ParameterDist_temp(1) = ParameterDist_temp(1) + epsilon;
leachsample_lambda = integral(@(t)fun(para, taugrid, t, taustep, x, y, ParameterDist_temp, nmixtures), 0, 1,'ArrayValued', true)';
lf_lambda = sum(log(leachsample_lambda))/N;
g_lambda = (lf_lambda-lf)/epsilon;

% C-4) A small disturbance to mu
epsilon = 0.001;
ParameterDist_temp = ParameterDist;
ParameterDist_temp(4) = ParameterDist_temp(4) + epsilon;
leachsample_mu = integral(@(t)fun(para, taugrid, t, taustep, x, y, ParameterDist_temp, nmixtures), 0, 1,'ArrayValued', true)';
lf_mu = sum(log(leachsample_mu))/N;
g_mu = (lf_mu-lf)/epsilon;

% C-4) A small disturbance to sigma
ParameterDist_temp = ParameterDist;
ParameterDist_temp(7) = ParameterDist_temp(7) + epsilon;
leachsample_sigma = integral(@(t)fun(para, taugrid, t, taustep, x, y, ParameterDist_temp, nmixtures), 0, 1,'ArrayValued', true)';
lf_sigma = sum(log(leachsample_sigma))/N;
g_sigma = (lf_sigma-lf)/epsilon;

%% Part D) gradients for lambda, mu, sigma
