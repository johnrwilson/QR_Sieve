function [lf,g] = gradllfnopreCovarpara(sv1,ntau, nsample, nmixtures, yt, X)
...
for j_sample = [1:nsample]
    ...
    leachsample= 1/ntau*sum( mnnoprevector(erroreachsample,ParameterDist,nmixtures) );    
    ...
    for j_mixture = [1:nmixtures]
        glambdaeachsample(j_mixture) = glambdaeachsample(j_mixture) + 1/ntau*sum(normpdf((erroreachsample-mu(j_mixture))/sigma(j_mixture)))/(sigma(j_mixture));
    end
    ...
    for j_mixture = [1:nmixtures]
        ...
        gmueachsample(j_mixture) = gmueachsample(j_mixture) + 1/ntau*sum(  lambda(j_mixture)*u_vector.*normpdf(u_vector)/((sigma(j_mixture))^2)  );
    end
    ...
    for j_mixture = [1:nmixtures]
     ...
     u_vector =  squeeze(u_matrix(j_mixture,:));
        gsigmaeachsample(j_mixture) = gsigmaeachsample(j_mixture) - 1/ntau*lambda(j_mixture)*sum( normpdf(u_vector) )/((sigma(j_mixture))^2);
        gsigmaeachsample(j_mixture) = gsigmaeachsample(j_mixture) + 1/ntau*lambda(j_mixture)*sum( (u_vector.^2).*normpdf(u_vector) )/((sigma(j_mixture))^2);
