% This code is the second step in calculating the coverage probability
% Haoyang Liu
% 9/17/2018
...
load Ye_GA_3mix_3_par_start_3_cover1_1 x1matrix x2matrix ytmatrix
x1r_raw = x1matrix;
x2r_raw = x2matrix;
y_raw = ytmatrix;
clear x1matrix x2matrix ytmatrix
...
parfor j_iter = [1:iter]
    ...
    x1r = x1r_raw(v_sample);
    x2r = x2r_raw(v_sample);
   ...
   % if  exit_recorder_qreg_start(j_iter) ~= -10
        ...
        if gradl_CDF_GA(start,  taugrid_temp, nmixtures, y', X') == Inf
            ...
            [fit_hat,fval,exitflag,output] =  ga(@(x)gradl_CDF_GA(x,  taugrid_temp, nmixtures, y', X'), nvars, A, b,[],[],lower_pl,upper_pl,[],opts);
...
%% 5) Save the result
save Ye_GA_3mix_3_par_start_3_cover2_2_1
return;
