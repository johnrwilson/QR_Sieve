% This code estimates a one normal EIV as a one normal 
% Haoyang Liu
% 3/5/2019
...
for j = [1:n_repeatsample]
    ...
    X_mean = X_mean_repeat(j,:);
    fit_hat = recorder_WLS_start_repeat(j,:);
    recorder_qreg = recorder_qreg_repeat(j,:);
    recorder_qreg_reshape = reshape(recorder_qreg,ntau,ncovar)';
    recorder_qreg_repeat_reshape(j,:,:) = recorder_qreg_reshape;
