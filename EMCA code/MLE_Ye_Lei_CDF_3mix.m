...
% It is also based on Lei's function so that we can use fmincon
% Haoyang Liu
% 3/14/2018
...
options=optimoptions(@fmincon,'display','iter','GradObj','on','UseParallel',true);
...
lower = [-5,repmat(-2,1,ntau),-5,repmat(-2,1,ntau),-5,repmat(-2,1,ntau),(zeros(1,(nmixtures-1))+0.001),(zeros(1,(nmixtures-1))-10.01),(zeros(1,nmixtures)+0.01)];
upper = [10,repmat(2,1,ntau),10,repmat(2,1,ntau),10,repmat(2,1,ntau),(zeros(1,(nmixtures-1))*0+1),(zeros(1,(nmixtures-1))+10),ones(1,nmixtures)*10];
...
%% 3) QREG
    ...
    fit_1_temp = fit_1';
    fit_1_temp(1, [2:(ntau+1)]) = fit_1_temp(1, [2:(ntau+1)]) - fit_1_temp(1, [1:ntau]);
    fit_1_temp(1, [(ntau+3) : (2*ntau+2)]) = fit_1_temp(1, [(ntau+3) : (2*ntau+2)]) - fit_1_temp(1, [(ntau+2) : (2*ntau+1)]);
    fit_1_temp(1, [(2*ntau+4) : (3*ntau+3)]) = fit_1_temp(1, [(2*ntau+4) : (3*ntau+3)])  - fit_1_temp(1, [(2*ntau+3) : (3*ntau+2)]) ;
    
    %fit_1_temp(1, 1) = 0;
    fit_1_temp(1, [2:(ntau+1)]) = 0.1;
    fit_1_temp(1, [(ntau+3) : (2*ntau+2)]) = 0.1;
    fit_1_temp(1, [(2*ntau+4) : (3*ntau+3)]) = 0.1;
...
%% 4) MLE
      ...
      display('reached MLE')  
      [fit_hat,fval,exitflag,output] = fmincon(@(x)gradl_CDF_Lei_GA(x,  taugrid, nmixtures, y', X'),start,A,b,[],[],lower,upper,[],options);
...
[b1_start,b2_start,b3_start,temp] = reconstruct_beta2(start([1:end-6]));
    
figure;
plot(taugrid,ma1,'k-o',taugrid,b1_start,'g-o',taugrid,zeros(1,length(taugrid)),'r-o',taugrid_qreg,mean(recorder_qreg([1:iter],1:11),1),'b-o');
legend('ours','start','true','q-reg','Location','northwest');
print('m1_3mix_fmincon_1','-dpng');

figure;
plot(taugrid,ma2,'k-o',taugrid,b2_start,'g-o',taugrid,exp(taugrid),'r-o',taugrid_qreg,mean(recorder_qreg([1:iter],12:22),1),'b-o');
legend('ours','start','true','q-reg','Location','northwest');
print('m2_3mix_fmincon_1','-dpng');

figure;
plot(taugrid,ma3,'k-o',taugrid,b3_start,'g-o',taugrid,sqrt(taugrid),'r-o',taugrid_qreg,mean(recorder_qreg([1:iter],23:33),1),'b-o');
legend('ours','start','true','q-reg','Location','northwest');
print('m3_3mix_fmincon_1','-dpng');
