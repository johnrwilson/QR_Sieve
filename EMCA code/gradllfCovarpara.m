function [lf,g] = gradllfCovarpara(parametervectorPre,ntau, nsample, nmixtures, nslice, yt,X)
...
if nslice >1
    parfor j_slice = [1:nslice]
        ...
        [lf_v(j_slice),g_v(:,j_slice)] = gradllfnopreCovarpara(parametervector,ntau, index_length(j_slice), nmixtures, yt(index_v), X(index_v,:));
    end
else
    j_slice = 1;
    index_v = ([begin_index(j_slice):end_index(j_slice) ]);
