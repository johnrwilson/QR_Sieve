...
close all 
  ...
  %%% Part A) Set Parameters and Load Data
    ...
    % 80 White 10 tau
    % load RData_pl_Angrist80_sub1_ntau15_black1  
    % 90 White 10 tau
    % load RData_pl_Angrist90_sub1_ntau19_black1  
    % 00 White 15 tau
    % load RData_pl_Angrist00_sub1_ntau17_black1 
    % 10 White 15 tau
    load RData_pl_Jacob10_sub1_ntau7_black1 
  ...
  %%% Part F) Plotting
    for j_covar = [1]
        ...
        title(sprintf('Data: %s. Number of knots %d. Black',postfixdata, (ntau-1)))
        print('-dpng','-r0',sprintf('beta%d_%s_black',(ntau-1),postfix));
    end
