...
recorder_WLS_All = nan(iter,n_WLS_iter,3*ntau);
...
for j_iter = [1:iter]
    ...
    if do_WLS == 1
        for j_WLS=1:n_WLS_iter
            ...
            recorder_WLS_All(j_iter,j_WLS,:) = [squeeze(betahat(1,:)),squeeze(betahat(2,:)),squeeze(betahat(3,:))];
        end
...
clear X Y weightV weightedX weightedy
if do_MLE_qreg_start == 1
    save Test_MLE_Residuals_Median_three_as3_logn_E_Vector16
end
