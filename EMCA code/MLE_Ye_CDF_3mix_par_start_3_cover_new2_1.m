...
load Ye_GA_3mix_3_par_start_3_cover_new_4
...
for j_iter = [1:iter]
    ...
    recorder_qreg_start_sorted_lower(j_iter,:) = prctile(recorder_qreg_start_sorted_temp,2.5,1);
    recorder_qreg_start_sorted_upper(j_iter,:) = prctile(recorder_qreg_start_sorted_temp,97.5,1);
    
    recorder_lambda_sorted_lower(j_iter,:) = prctile(recorder_lambda_sorted_temp,2.5,1);
    recorder_lambda_sorted_upper(j_iter,:) = prctile(recorder_lambda_sorted_temp,97.5,1);

        
    recorder_mu_sorted_lower(j_iter,:) = prctile(recorder_mu_sorted_temp,2.5,1);
    recorder_mu_sorted_upper(j_iter,:) = prctile(recorder_mu_sorted_temp,97.5,1);
    
    recorder_sigma_sorted_lower(j_iter,:) = prctile(recorder_sigma_sorted_temp,2.5,1);
    recorder_sigma_sorted_upper(j_iter,:) = prctile(recorder_sigma_sorted_temp,97.5,1);
