...
close all 
  ...
  %%% Part A) Set Parameters and Load Data
    ...
    postfix =     'Angrist90'; %    'Jacob10' %  
    postfixdata =   'Angrist 90'; %   'Jacob 10'; % 
    
    % 80 White 15 tau
    % load RData_pl_Angrist80_sub1_ntau15_white1_ue 
    % 90 White 15 tau
    load RData_pl_Angrist90_sub1_ntau33_white1_ue_while
    % 00 White 15 tau
    % load RData_pl_Angrist00_sub1_ntau15_white1_ue
    % 10 White 15 tau
    % load RData_pl_Jacob10_sub1_ntau15_white1_ue
  ...
  %%% Part C) Create "taugrid_break_sorted", which repeats each middle entry twice 
    taugrid_break_sorted = repeat_HL(taugrid_midpoint,2);
  ...
  %%% Part F) Plotting
    for j_covar = [1]
        ...
        plot(taugrid_midpoint,beta_qreg(j_covar+1,:),'g');
