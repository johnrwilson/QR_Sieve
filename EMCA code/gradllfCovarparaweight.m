function [lf,g] = gradllfCovarparaweight(parametervectorPre,ntau, nsample, nmixtures, nslice, yt,X,weight)
...
parametervector = [parametervectorPre(1:ntau*ncovar),lambda,mu,sigma];
 ...
 parfor j_slice = [1:nslice]
     ...
     [lf_v(j_slice),g_v(:,j_slice)] = gradllfnopreCovarparaweight(parametervector,ntau, index_length(j_slice), nmixtures, yt(index_v), X(index_v,:),weight);
 end
