function [lf,g] = gradllfnopre_JY(sv1,ntau, nsample, nmixtures, yt, x2r)
    ...
    beta1=sv1(1:ntau);
    beta2=sv1((ntau+1):(2*ntau));

    ParameterDist=sv1((2*ntau+1):(2*ntau+3*nmixtures));
    ...
    for j_sample = [1:nsample]
        ...
        for j_tau = [1:ntau]
            erroreachsample(j_tau) = (yt(j_sample)-x2.*beta2(j_tau)-beta1(j_tau));
        end
       ...
       gbeta2eachsample = zeros(ntau,1);
        ...
        gbeta1eachsample = gbeta1eachsample*(-1/ntau)*1/leachsample;    
    ...
    g = [gbeta1;gbeta2;glambda;gmu;gsigma];
