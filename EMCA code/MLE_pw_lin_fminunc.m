% This codes implements the minimization using fminunc
% Haoyang Liu
...
% Option for fminunc
options = optimoptions('fminunc','display','iter','SpecifyObjectiveGradient',true);%'Algorithm','trust-region',
...
%% 4) MLE
     ...
     if llf_start == Inf
      ...
      display('reached MLE fminunc')
      [fit_hat,fval,exitflag,output] = fminunc(@(x)gradl_search_Lei(x,taugrid,nmixtures,y',X'),start,options);
...
switch truth_start
    case -5
        save (sprintf('pl_unc_truth_b2_5_%d',ntau))
    case -4
        save (sprintf('pl_unc_truth_c2_%d',ntau))

    case -2
        save (sprintf('pl_unc_qreg_para_%d',ntau))
    case -1
        save (sprintf('pl_unc_truth_para_%d',ntau))
    case 1
        save (sprintf('pl_unc_truth_%d',ntau))
    otherwise
        save (sprintf('pl_unc_qreg_%d',ntau))
end
