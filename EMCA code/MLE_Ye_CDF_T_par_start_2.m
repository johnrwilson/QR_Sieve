...
lower=[zeros(1,(3*ntau))-3, (zeros(1,(nmixtures-1))+0.001),(zeros(1,(nmixtures-1))-10.01),(zeros(1,nmixtures)+0.01)];
upper=[(zeros(1,(3*ntau))+6), (zeros(1,(nmixtures-1))*0+1),(zeros(1,(nmixtures-1))+10),ones(1,nmixtures)*10];
...
parfor j_iter = [1:iter]
    ...
    %recorder_ParaDist = [[1,0],[0,0],[2.5,1,1]];
    ...
    if gradllfCovarparavector(start, ntau, nsample, nmixtures,1, y, X) == Inf
        ...
        [recorder_qreg_start(j_iter,:),fval,exitflag] = fmincon(@(x)gradllfCovarparavector(x, ntau, nsample, nmixtures,1,y, X),start,A,b,[],[],lower,upper,[],options);
        fval_recorder_qreg_start(j_iter) = fval;
        fit_hat_pc = recorder_qreg_start(j_iter,:);
    ...
    if  exit_recorder_qreg_start(j_iter) ~= -10
        display('reached MLE (piecewise linear)');
        
        X_mean = mean(X);
        recorder_WLS_start_reshape = reshape(fit_hat_pc(1,1:(ntau)*ncovar),(ntau),ncovar);
        ...
        start = [fit_1_temp, fit_hat_pc([(end - 3*nmixtures + 3) : end])];
        ...
        if gradl_CDF_GA(start,  taugrid_temp, nmixtures, y', X') == Inf
            ...
            [fit_hat_pl,fval_pl,exitflag_pl] =  ga(@(x)gradl_CDF_GA(x,  taugrid_temp, nmixtures, y', X'), nvars, A, b,[],[],lower_pl,upper_pl,[],opts);
            recorder_pc_start(j_iter,:) = fit_hat_pl;
            fval_recorder_pc_start(j_iter) = fval_pl;
            exit_recorder_pc_start(j_iter) = exitflag_pl;
        end
   end
