% Piecewise linear MLE on white subsamples
% This version
%
% Updated 8/16/2018
...
% data could be Angrist80 to Angrist00, or Jacob 10
...
% GA, starting from fmincon result
recorder_fmincon_start_repeat = nan(n_repeatsample,nvars);
fval_recorder_fmincon_start_repeat = nan(1,n_repeatsample);
exit_recorder_fmincon_start_repeat = nan(1,n_repeatsample);
...
for j = [1:n_repeatsample]   
    ...
    [fit_hat,fval,exitflag,output] = fmincon(@(x)gradl_CDF_Lei_GA(x,  taugrid_temp, nmixtures, y', X'), start, A, b,[],[],lower_pl,upper_pl,[],options);
    recorder_pc_start_repeat(j,:) = start;
%    recorder_pc_start_repeat(j,:) = fit_hat;
%   fval_recorder_pc_start_repeat(j) = fval;
%    exit_recorder_pc_start_repeat(j) = exitflag;

    
%     opts.InitialPopulationMatrix = fit_hat;
%     [fit_hat,fval,exitflag,output] = ga(@(x)gradl_CDF_GA(x,  taugrid_temp, nmixtures, y', X'), nvars_pl, A, b, [],[],lower_pl,upper_pl,[],[],opts);
%     recorder_fmincon_start_repeat(j,:) = fit_hat;
%     fval_recorder_fmincon_start_repeat(j) = fval;
%     exit_recorder_fmincon_start_repeat(j) = exitflag;
...
save (sprintf('RData_pl_%s_sub%d_ntau%d_white%d_new',postfix,n_subsample,ntau,n_repeatsample))
